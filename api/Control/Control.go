...
func (h *Handler) ForceShutDownVM(w http.ResponseWriter, r *http.Request) {
	...
	if err := httputil.HttpDecoder(r, param); err != nil {
		...
		h.Logger.Error("failed to decode forceShutdown request", zap.Error(ERR))
		return
	}
	...
	if err := termination.DomainTerminatorFactory(dom).TerminateDomain(); err != nil {
		resp.ResponseWriteErr(w, virerr.ErrorJoin(err, fmt.Errorf("error shutting down vm")), http.StatusInternalServerError)
		return
	}

	if err := h.DomainControl.SleepDomain(dom, h.Logger); err != nil {
		ERR := virerr.ErrorJoin(err, fmt.Errorf("error getting domain status for forceShutdown"))
		resp.ResponseWriteErr(w, ERR, http.StatusInternalServerError)
		h.Logger.Error("failed to sleep domain for forceShutdown", zap.String("uuid", param.UUID), zap.Error(ERR))
		return
	}
...
func (h *Handler) DeleteVM(w http.ResponseWriter, r *http.Request) {
	...
	if err := httputil.HttpDecoder(r, param); err != nil {
		...
		resp.ResponseWriteErr(w, ERR, http.StatusInternalServerError)
		h.Logger.Error("failed to decode deleteVM request", zap.Error(ERR))
		return
	}
	...
	if err := termination.DomainDeleterFactory(domain, param.DeletionType, param.UUID).DeleteDomain(); err != nil {
		ERR := virerr.ErrorJoin(err, fmt.Errorf("error deleting vm"))
		resp.ResponseWriteErr(w, ERR, http.StatusInternalServerError)
		h.Logger.Error("failed to delete domain", zap.String("uuid", param.UUID), zap.Error(ERR))
		return
	}

	if err := h.DomainControl.RemoveDomain(domain, param.UUID, h.Logger); err != nil {
		ERR := virerr.ErrorJoin(err, fmt.Errorf("error removing domain from list"))
		resp.ResponseWriteErr(w, ERR, http.StatusInternalServerError)
		h.Logger.Error("failed to remove domain", zap.String("uuid", param.UUID), zap.Error(ERR))
		return
	}
